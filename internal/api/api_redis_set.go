...
import (
	...
	"github.com/raoqu/gojs/internal/util/strings"
...
func Redis_sadd(rt *goja.Runtime, call goja.FunctionCall) (goja.Value, error) {
	...
	count, err := util.RedisData.SAdd(context.Background(), key, members...).Result()
	return toGojaResult(rt, count, err)
}
...
func Redis_srem(rt *goja.Runtime, call goja.FunctionCall) (goja.Value, error) {
	...
	count, err := util.RedisData.SRem(context.Background(), key, members...).Result()
	return toGojaResult(rt, count, err)
}
...
func Redis_scard(rt *goja.Runtime, call goja.FunctionCall) (goja.Value, error) {
	...
	count, err := util.RedisData.SCard(context.Background(), key).Result()
	return toGojaResult(rt, count, err)
}
...
func Redis_smembers(rt *goja.Runtime, call goja.FunctionCall) (goja.Value, error) {
	...
	members, err := util.RedisData.SMembers(context.Background(), key).Result()
	return toGojaResult(rt, members, err)
}
